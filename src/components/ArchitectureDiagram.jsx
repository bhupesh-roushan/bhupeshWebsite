...
export const ArchitectureDiagram = ({ id }) => {
  ...
  const { layers, store, note } = spec;
  const rowY = layers.map((_, i) => TOP + i * (BOX_H + GAP_Y));
  const lastRowBottom = rowY[rowY.length - 1] + BOX_H;
  // Not every project has a database. The client-only ones would otherwise get
  // an empty persistence band and a dangling arrow into nothing.
  const storeY = store ? lastRowBottom + GAP_Y + 6 : null;
  const height = (store ? storeY + 34 : lastRowBottom) + 8;
  const marker = `arrow-${id}`;

  return (
    <div>
      {/* Only the drawing scrolls. Sized to a phone-width modal the whole
          880-unit frame scales to ~0.32, putting box labels under 5px — but
          the note must stay put, or it slides off as you pan the diagram. */}
      <div className="overflow-x-auto">
        <svg
          ...
          aria-label={`${id} architecture: ${layers
            ...
            .join(" then ")}${store ? `, persisting to ${store}` : ""}.`}
        >
          <defs>
            <marker
              id={marker}
      ...
      {/* The one thing the boxes can't show: what the arrows actually carry, or
          what the shape costs. Boxes name the parts; this says how it works. */}
      {note && (
        <p className="mt-3 border-l-2 border-white/15 pl-3 text-xs leading-relaxed text-gray-400">
          {note}
        </p>
      )}
    </div>

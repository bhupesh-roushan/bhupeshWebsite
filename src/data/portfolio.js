...
export const projects = [
  {
    ...
    summary:
      "Grades student submissions against per-question rubrics, then writes the scores back to the source spreadsheet in the previous grader's format. Runs against whichever model an operator registers — a self-hosted vLLM pod, local Ollama, OpenAI, Claude or OpenRouter — and anything it flags goes to a human review queue before it counts.",
    ...
    bullets: [
      "Built an admin-only platform that grades student submissions against per-question rubrics, covering both written answers and GitHub repositories, with the model endpoint registered by an operator rather than hardcoded to one provider.",
      "Wrote a single adapter over six backends — vLLM, Ollama, OpenAI, Claude, OpenRouter and a mock for GPU-free local development — collapsing three different request shapes behind one call, so moving from a rented GPU pod to a hosted API is a settings change rather than a code change.",
      "Normalised what the adapter couldn't hide: reasoning is a visible mode on vLLM, Ollama and OpenRouter but happens server-side on Claude and OpenAI's o-series, and JSON-schema output is withheld from OpenRouter because support varies by whichever upstream it routes to.",
      ...
      "Routed every request to the healthy endpoint with the fewest in-flight jobs, tracked spend per run against a budget cap using provider-reported cost where it's available, and requeued the jobs of any pod that died — rented pods are preemptible, so losing one mid-batch is expected rather than exceptional.",
